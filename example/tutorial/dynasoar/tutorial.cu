...
int main(int argc, char** argv)
{
  ...
  allocator_handle->parallel_do<Fib, &Fib::calc>();
  allocator_handle->parallel_do<Fib, &Fib::print_n>();
}

__global__ void do_calc(int n)
{
  new(device_allocator) Fib(n);
}
...
__device__ void Fib::calc()
{
  ...
  Sum* sum = new(device_allocator) Sum();
  new(device_allocator) Fib(n - 1);
  new(device_allocator) Fib(n - 2);
}
